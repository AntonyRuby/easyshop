...
class CartController extends GetxController implements GetxService {
  ...
  void updateOrAdd(Item item) {
    final firstItem = _cartList.isEmpty ? item : _cartList.first.item!;
    if (item.storeId != firstItem.storeId) return;

    for (final cart in _cartList) {
      if (cart.item!.id == item.id) {
        updateCartQuantityOnline(cart.id!, cart.price!, cart.quantity! + 1);
        return;
      }
    }
