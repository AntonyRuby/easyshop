...
class _WalletScreenState extends State<WalletScreen> {
      ...
      if ((widget.fundStatus == 'success' || widget.fundStatus == 'fail' || widget.fundStatus == 'cancel') &&
        ...
        Future.delayed(const Duration(seconds: 2), () {
          Get.showSnackbar(GetSnackBar(
            backgroundColor: widget.fundStatus == 'fail' || widget.fundStatus == 'cancel' ? Colors.red : const Color(0xFFFE0100),
        ...
        if (scrollController.position.pixels == scrollController.position.maxScrollExtent &&
          ...
          if (Get.find<WalletController>().offset < pageSize) {
            ...
            Get.find<WalletController>().getWalletTransactionList(
                Get.find<WalletController>().offset.toString(), false, widget.fromWallet, Get.find<WalletController>().type);
          }
        }
      });
    }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      floatingActionButton: isLoggedIn && !ResponsiveHelper.isDesktop(context) && !widget.fromWallet
                ...
                Get.dialog(
                  Dialog(
                      ...
                      child: WalletBottomSheet(
                        ...
                        amount: Get.find<UserController>().userInfoModel!.loyaltyPoint == null
                            ? '0'
                            : Get.find<UserController>().userInfoModel!.loyaltyPoint.toString(),
                      )),
                );
              },
            )
