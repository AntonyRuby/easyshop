...
class _ParcelLocationScreenState extends State<ParcelLocationScreen>
  ...
  Future<String> splitPhoneNumber(String number, bool returnCountyCode) async {
    ...
    if (GetPlatform.isAndroid || GetPlatform.isIOS) {
      try {
        ...
        Get.find<ParcelController>()
            .setCountryCode(phoneNumber.countryCode, false);
      } catch (_) {}
        ...
        onPressed: () async {
          if (_tabController!.index == 0) {
            ...
            String numberWithCountryCode =
                '${GetPlatform.isWeb ? '' : '+'}${parcelController.receiverCountryCode!}${_receiverPhoneController.text.trim()}';
  ...
  Future<void> _validateSender(ParcelController parcelController) async {
    String numberWithCountryCode =
        '${GetPlatform.isWeb ? '' : '+'}${parcelController.senderCountryCode ?? ''}${_senderPhoneController.text.trim()}';
    PhoneValid phoneValid =
        await CustomValidator.isPhoneValid(numberWithCountryCode);
    numberWithCountryCode = phoneValid.phone;
