...
class _OfflinePaymentScreenState extends State<OfflinePaymentScreen> {
  PageController pageController = PageController(
      ...
      initialPage: Get.find<OrderController>().selectedOfflineBankIndex);
  ...
  Future<void> initCall() async {
    if (widget.forParcel) {
      ...
      Get.find<OrderController>().selectOfflineBank(
          Get.find<ParcelController>().selectedOfflineBankIndex,
          canUpdate: false);
      Get.find<OrderController>().changesMethod(canUpdate: false);
    }
    ...
    if (Get.find<OrderController>().offlineMethodList != null &&
        Get.find<OrderController>().offlineMethodList!.isNotEmpty) {
      for (int index = 0;
          index <
              Get.find<OrderController>()
                  .offlineMethodList![
                      Get.find<OrderController>().selectedOfflineBankIndex]
                  .methodInformations!
                  .length;
          index++) {
        Get.find<OrderController>()
    ...
    return Scaffold(
      ...
      body: SafeArea(
        child: GetBuilder<OrderController>(builder: (orderController) {
          List<MethodInformations>? methodInformation = orderController
                      .offlineMethodList !=
                  null
              ? orderController
                  .offlineMethodList![orderController.selectedOfflineBankIndex]
                  .methodInformations!
              : [];

          return orderController.offlineMethodList != null
              ? Column(children: [
                  Expanded(
                        ...
                        child:
                          ...
                          SizedBox(
                            ...
                            child: PageView.builder(
                                ...
                                itemCount:
                                    orderController.offlineMethodList!.length,
                                itemBuilder: (context, index) {
                                  bool selected = orderController
                          ...
                          ResponsiveHelper.isDesktop(context)
                              ? completeButton(
                                  orderController, methodInformation)
                              : const SizedBox(),
                        ]),
                      ),
                    ),
                  ...
                  !ResponsiveHelper.isDesktop(context)
                      ? completeButton(orderController, methodInformation)
                      : const SizedBox(),
                ])
    ...
    return Padding(
      ...
      child: CustomButton(
        ...
        onPressed: () async {
          ...
          if (complete) {
            String methodId = orderController
                .offlineMethodList![orderController.selectedOfflineBankIndex].id
                .toString();
              ...
              for (int i = 0; i < methodInformation.length; i++) {
                data.addAll({
                  methodInformation[i].customerInput!:
                      orderController.informationControllerList[i].text,
                });
              }
            ...
            itemBuilder: (context, i) {
              return Padding(
                ...
                child: Row(children: [
                  ...
                  Text(offlineMethodList[index].methodFields![i].inputData!,
                      style: robotoMedium),
                ]),
              );
            })
