...
class CheckoutScreenState extends State<CheckoutScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      body: guestCheckoutPermission || Get.find<AuthController>().isLoggedIn()
          ? GetBuilder<LocationController>(builder: (locationController) {
              return GetBuilder<StoreController>(builder: (storeController) {
                List<DropdownItem<int>> addressList =
                    CheckoutHelper.getDropdownAddressList(
                        context: context,
                        addressList: locationController.addressList,
                        store: storeController.store);
                address = CheckoutHelper.getAddressList(
                    addressList: locationController.addressList,
                    store: storeController.store);
                ...
                Pivot? moduleData =
                    CheckoutHelper.getModuleData(store: storeController.store);
                _isCashOnDeliveryActive =
                    CheckoutHelper.checkCODActive(store: storeController.store);
                _isDigitalPaymentActive =
                    CheckoutHelper.checkDigitalPaymentActive(
                        store: storeController.store);
                _isOfflinePaymentActive = Get.find<SplashController>()
                        .configModel!
                        .offlinePaymentStatus! &&
                    CheckoutHelper.checkZoneOfflinePaymentOnOff(
                        addressModel:
                ...
                return GetBuilder<CouponController>(
                  ...
                  return GetBuilder<OrderController>(
                    ...
                    double price = CheckoutHelper.calculatePrice(
                        store: storeController.store, cartList: _cartList);
                    double addOns = CheckoutHelper.calculateAddonsPrice(
                        store: storeController.store, cartList: _cartList);
                    double variations = CheckoutHelper.calculateVariationPrice(
                        store: storeController.store,
                        cartList: _cartList,
                    ...
                    double subTotal = CheckoutHelper.calculateSubTotal(
                        price: price,
                        addOns: addOns,
                        variations: variations,
                        cartList: _cartList);
                    ...
                    double originalCharge =
                        CheckoutHelper.calculateOriginalDeliveryCharge(
                      store: storeController.store,
                      address: locationController.getUserAddress()!,
                      distance: orderController.distance,
                      extraCharge: orderController.extraCharge,
                    );
                    double deliveryCharge =
                        CheckoutHelper.calculateDeliveryCharge(
                      store: storeController.store,
                      address: locationController.getUserAddress()!,
                      distance: orderController.distance,
                      extraCharge: orderController.extraCharge,
            ...
            onPressed: orderController.acceptTerms
                ? () {
                    ...
                    if (isGuestLogIn &&
                      ...
                      if (isGuestLogIn &&
                          orderController.orderType == 'take_away') {
                        String number = orderController.countryDialCode! +
                        ...
                        finalAddress = AddressModel(
                          ...
                          longitude: Get.find<LocationController>()
                              .getUserAddress()!
                              .longitude,
                          zoneId: Get.find<LocationController>()
                              .getUserAddress()!
                              .zoneId,
                      ...
                      if (widget.storeId == null) {
                        ...
                        PlaceOrderBody placeOrderBody = PlaceOrderBody(
                          ...
                          orderAmount: total,
                          orderNote:
                              Get.find<OrderController>().noteController.text,
                          orderType: orderController.orderType,
                        ...
                        if (orderController.paymentMethodIndex == 3) {
                          Get.toNamed(RouteHelper.getOfflinePaymentScreen(
                              ...
                              zoneId: storeController.store!.zoneId!,
                              total: /*total*/ orderController.viewTotalPrice!,
