  ...
  Widget build(BuildContext context) {
    return GetBuilder<AuthController>(builder: (authController) {
      ...
      if (authController.moduleList != null &&
          authController.moduleList!.isNotEmpty) {
        for (int index = 0;
            index < authController.moduleList!.length;
            index++) {
          if (authController.moduleList![index].moduleType != 'parcel') {
            moduleIndexList.add(index);
            moduleList.add(DropdownItem<int>(
                value: index,
                child: SizedBox(
                  child: Padding(
                    padding: const EdgeInsets.all(8.0),
                    child:
                        Text('${authController.moduleList![index].moduleName}'),
                  ),
                )));
          }
        }
      }
      return authController.moduleList != null
          ? Container(
